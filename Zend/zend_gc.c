...
#define RESUMED_AFTER_SUSPENSION (fiber != NULL && GC_G(dtor_fiber) != fiber) \
	|| (in_coroutine && GC_G(dtor_coroutine) != ZEND_CURRENT_COROUTINE)
#else
#define RESUMED_AFTER_SUSPENSION fiber != NULL && GC_G(dtor_fiber) != fiber
#endif
	...
	while (idx != end) {
		...
		if (GC_IS_DTOR_GARBAGE(current->ref)) {
			...
			if (!(OBJ_FLAGS(p) & IS_OBJ_DESTRUCTOR_CALLED)) {
				...
				GC_ADDREF(obj);
#ifdef PHP_ASYNC
				ZEND_CURRENT_COROUTINE->extended_data = obj;
#endif
				obj->handlers->dtor_obj(obj);
#ifdef PHP_ASYNC
				ZEND_CURRENT_COROUTINE->extended_data = NULL;
#endif
...
ZEND_API int zend_gc_collect_cycles(void)
{
	...
	if (UNEXPECTED(ZEND_IS_ASYNC_ON && ZEND_CURRENT_COROUTINE != GC_G(dtor_coroutine))) {
